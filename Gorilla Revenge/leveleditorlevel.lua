...
function new()	
	...
	local jsonLevel = loadValue("templevel.data")
	local levelTable = json.decode(jsonLevel)
	...
	local function callNextLevel()
		...
		if winTime < 20 then
			saveValue("level" .. currentLevel .. "rating.data","3")
		elseif winTime < 30 then
			saveValue("level" .. currentLevel .. "rating.data","2")
		else
			saveValue("level" .. currentLevel .. "rating.data","1")
		end
		...
		if onGround then
			if gorilla.direction == "right" then
				transition.to(gorilla, {time = 500, alpha = 0})
			else
				transition.to(gorillaLeft, {time = 500, alpha = 0})
			end
		else
			if gorilla.direction == "right" then
				transition.to(jumpRightCover, {time = 500, alpha = 0})
			else
				transition.to(jumpLeftCover, {time = 500, alpha = 0})
			end
		end
		timer.performWithDelay(500, director:changeScene("leveleditorfinishedscreen"), 1)
	end
	...
	local function createItemsForEveryLevel()
		...
		local onPauseRelatedButtonTouch = function( event )
			if event.phase == "release" then	
				...
				elseif event.id == "editorbutton" then
					...
					saveValue("level-to-load.data", jsonLevel)
		...
		editorBtn = ui.newButton{
			defaultSrc = "images/editor-button.png",
			defaultX = 80,
			defaultY = 44,
			overSrc = "images/editor-button-pressed.png",
			overX = 80,
			overY = 44,
			onEvent = onPauseRelatedButtonTouch,
			id = "editorbutton",
		...
		editorBtn.x = 240; editorBtn.y = 200;
		editorBtn.alpha = .7
		pauseGroup:insert( editorBtn )
